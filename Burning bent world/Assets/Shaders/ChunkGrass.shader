Shader "Custom/ChunkGrass"
{
	Properties
	{
		_GrassBaseColor("Base Color", 2D) = "" {}
		_GrassTipColor("Tip Color", 2D) = "" {}
		_BladeTexture("Blade Texture", 2D) = "white" {}

		_ChunkX("Chunk X (Chunk.Size * Chunk.Size)", Int) = 0
		_ChunkZ("Chunk Z (Chunk.Size * Chunk.Size)", Int) = 0
	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			CBUFFER_START(UnityPerMaterial)
				sampler2D _GrassBaseColor;
				sampler2D _GrassTipColor;
        ...
        Pass
        {
            ...
            float4 frag (GeomData i) : SV_Target
            {
            	...
            	// Sample color of the cell which corresponds to the chunk x and y
				const float4 base_color = tex2D(_GrassBaseColor, float2(
					(i.worldPos.x - _ChunkX) / 16.0f,
					(i.worldPos.z - _ChunkZ) / 16.0f
				));
				const float4 tip_color = tex2D(_GrassTipColor, float2(
					(i.worldPos.x - _ChunkX) / 16.0f,
					(i.worldPos.z - _ChunkZ) / 16.0f
				));
                ...
                return color * lerp(base_color, tip_color, i.uv.y);
			}
